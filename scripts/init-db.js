...
if (!listColumns.rows.some((row) => row.name === 'description')) {
	await db.execute("ALTER TABLE lists ADD COLUMN description TEXT NOT NULL DEFAULT ''");
	console.log('Added `description` column to lists.');
}
